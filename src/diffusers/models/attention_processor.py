    ...
    def __call__(
        ...
        **kwargs,
        ...
        def custom_scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0,
            ...
            # TODO: device is manually set
            attn_bias = torch.zeros(L, S, dtype=query.dtype, device=torch.device('cuda'))
            if is_causal:
                assert attn_mask is None
                temp_mask = torch.ones(L, S, dtype=torch.bool, device=torch.device('cuda')).tril(diagonal=0)
            ...
            attn_weight_props = torch.softmax(attn_weight, dim=-1)
            attn_weight = torch.dropout(attn_weight_props, dropout_p, train=True)
            return attn_weight @ value, attn_weight_props
        
        hidden_states, softmax_output = custom_scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )
        ...
        return hidden_states, softmax_output
